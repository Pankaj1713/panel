...
const Dashboard = () => {
  ...
  const validateForm = () => {
    ...
    if (!name) {
      ...
      toast.error(errors.name, toastConfig); // Show toast error
      valid = false;
    }
    if (!description_content) {
      errors.description_content = "Description is required.";
      toast.error(errors.description_content, toastConfig); // Show toast error
      valid = false;
    }
    if (!price || isNaN(price) || price <= 0) {
      errors.price = "Please enter a valid price.";
      toast.error(errors.price, toastConfig); // Show toast error
      valid = false;
    }
    if (!category) {
      errors.category = "Category is required.";
      toast.error(errors.category, toastConfig); // Show toast error
      valid = false;
    }
    if (!image) {
      errors.image = "Image is required.";
      toast.error(errors.image, toastConfig); // Show toast error
      valid = false;
    }
  ...
  const handleSubmitEdit = async () => {
    ...
    try {
      ...
      const updatedForm = { ...form, _id: editingProductId };
      
      await axios.put(
        `https://cj-backend.onrender.com/product/${editingProductId}`,
        updatedForm // Send the updated form with the id
      );
